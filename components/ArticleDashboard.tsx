...
export default function ArticleDashboard() {
  ...
  }, [f.activeArr.join(","), f.status, f.paywall, f.atype, f.author, f.topics.join(","), f.lang, f.changed, f.depth, f.hideRegional, f.linkState, f.onlineCut, f.rangeFrom, f.rangeTo, f.timeAxis, f.pinpoint?.sourceId, f.pinpoint?.topic, f.pinpoint?.limit, f.kwIds, f.keyword, f.subPats.join("|"), page, tableSort?.key, tableSort?.dir]);

  useEffect(() => { loadRows(); }, [loadRows]);
  useEffect(() => { setPage(0); }, [f.activeArr.join(","), f.status, f.paywall, f.atype, f.author, f.topics.join(","), f.subPats.join("|"), f.keyword, f.lang, f.changed, f.depth, f.hideRegional, f.linkState, f.onlineCut, f.rangeFrom, f.rangeTo, f.timeAxis, f.pinpoint?.sourceId, f.pinpoint?.topic, tableSort?.key, tableSort?.dir]);
