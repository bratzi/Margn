...
export default function TimeRangeFilter() {
  ...
  useEffect(() => {
    ...
    const move = (e: PointerEvent) => {
      ...
      if (drag === "from") setRangeIdx({ from: Math.min(i, rangeIdx.to - 1), to: rangeIdx.to });
      else if (drag === "to") setRangeIdx({ from: rangeIdx.from, to: Math.max(i, rangeIdx.from + 1) });
  ...
  return (
    <div className="trf trf-open" style={{ height: h }}>
      ...
      <div className="trf-head">
        <div className="trf-title">Veröffentlichungs-Zeitraum <span className="trf-range">{fmtDay(days[rangeIdx.from])} – {fmtDay(days[rangeIdx.to])}</span></div>
        <div className="trf-legend">{series.map((s) => <span key={s.id}><i style={{ background: s.color }} />{nameById.get(s.id)}</span>)}</div>
        {(rangeIdx.from > 0 || rangeIdx.to < N - 1) && <button className="trf-reset" onClick={() => setRangeIdx({ from: 0, to: N - 1 })}>Zurücksetzen</button>}
      ...
      <div className="trf-chart" ref={trackRef} style={{ height: chartH }}>
        ...
        <div className="trf-dim" style={{ left: 0, width: `${pctOf(rangeIdx.from)}%` }} />
        <div className="trf-dim" style={{ right: 0, width: `${100 - pctOf(rangeIdx.to)}%` }} />
        <div className="trf-bandsel" style={{ left: `${pctOf(rangeIdx.from)}%`, width: `${pctOf(rangeIdx.to) - pctOf(rangeIdx.from)}%` }} onPointerDown={(e) => start("band", e)} />
        <div className="trf-h" style={{ left: `${pctOf(rangeIdx.from)}%` }} onPointerDown={(e) => start("from", e)}><span /></div>
        <div className="trf-h" style={{ left: `${pctOf(rangeIdx.to)}%` }} onPointerDown={(e) => start("to", e)}><span /></div>
      </div>
