...
-- Gebündelter Abruf fürs Dashboard: Minuten- oder Stunden-Körnung, Summe je Quelle.
...
language sql stable as $$
  select s.source_id,
         date_trunc(case when p_gran = 'minute' then 'minute' else 'hour' end, s.minute) as bucket,
         sum(s.n)::bigint as n
  from sightings s
